...
board = pygame.display.set_mode((SURF_B, SURF_H)) 
...
#tracking each counter on the board(0-white, 1-black, 2-nothing)
WHITE = 0
BLACK = 1
EMPTY = 2 # TODO: -1?
counters = []
...
def fill_board():
    board.blit(board_pic, (0, 0))
    board.blit(cells, (0, 0))
    for row in range(8):
        for col in range(8):
            # blit the counter on the board for each cell
            if counters[row][col] == WHITE:
                board.blit(wh_counter, (row*cell_b, col*cell_h))
            if counters[row][col] == BLACK:
                board.blit(bl_counter, (row*cell_b, col*cell_h))
    pygame.display.update()
...
def show_score():
    ...
    '''if turn == WHITE:
        ...
        player = 'BLACK'
        '''
    num_done = score[WHITE]+score[BLACK]
    wh_score = font.render(str(score[WHITE]), True, text_col)
    bl_score = font.render(str(score[BLACK]), True, text_col)
    progress = font.render("Game progress", True, text_col)
    #playing = font.render(player+"'s turn!", True, salmon)
    pygame.draw.rect(board, bg, (640, 0, 160, 640))
    pygame.draw.rect(board, text_col, (640, 0, 160, 640), 5)
    pygame.draw.rect(board, text_col, (660, 20+7*cell_h, num_done*120//64, cell_h//4))
    pygame.draw.rect(board, text_col, (660, 20+7*cell_h, 120, cell_h//4), 1)
    board.blit(wh_score, (660, 20+cell_h))
    board.blit(bl_score, (660, 20+3*cell_h))
    board.blit(progress, (660, 7*cell_h))
    #board.blit(playing, (660, 20+4*cell_h))
    board.blit(menu_button, (660, 50+5*cell_h))
    board.blit(wh_counter, (660, 10))
    board.blit(bl_counter, (660, 10+2*cell_h))
    if turn == WHITE: #TODO: Use same color to indicate the turn
        pygame.draw.ellipse(board, wh_ring, (660+4, 10+3, cell_b-6, cell_h-6), 5) #magenta, 
    else:
        pygame.draw.ellipse(board, bl_ring, (660+3, 10+2*cell_h+3, cell_b-5, cell_h-5), 5)


#function to check if the board is full
def game_over():
    for i in range(8):
        if EMPTY in counters[i]:
            return False
    return True
...
def reset():
    global counters, score, turn, num_done
    counters = default[::]
    score = [0, 0]
    turn = WHITE
    num_done = 0
...
board.blit(board_pic, (0, 0))   # TODO: blit and update can be refactored into a function 
board.blit(cells, (0, 0))       # TODO: board and cells are always blitted together. Can be refactored into a function draw_board() or something
show_score()
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if not(menu_on):
                if x_cell < 8 and y_cell < 8:
                    if counters[x_cell][y_cell] == 2:
                        counters = change_board(counters, x_cell, y_cell)
                        fill_board()
